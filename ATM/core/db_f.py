import os
import logging
from ATM.config.settings import db_path

logger = logging.getLogger(__name__)
db_dic = {"emp": "id,login,pwd,name,limit,money,age,phone,count,date",
          "pay": "id,login,commodity,num,total,date",
          "bill": "id,login,describe,amt,type,date",
          "bs": "id,login,pwd,name,phone,count,date"
          }


def sql_distribute(sql_str, re_value=None):
    func = {
    ...
    if isinstance(sql_str, list):
        for sql_s in sql_str:
            re_value = sql_func(sql_s, func)
        return re_value
    else:
        return sql_func(sql_str, func)


def sql_func(sql_s, func):
    sql_s = sql_s.split()
    key = sql_s[0]
    if key in func:
        logger.warning("进入%s功能，调用%s子功能" % (sql_distribute.__name__, func[key].__name__))
        re_value = func[key](sql_s)
    else:
        return print('输入错误')
    return re_value
    ...
    if select_dict['select'][0] != "*":
        ...
        for tag in select_dict['select']:
            if tag in db_dict:
                ...
                return logger.error("查询条件不在字典中请重新输入")
        re_list.append(v)
    elif select_dict['select'][0] == "*":
        line = line.split(',')
        db_dict = dict(zip(db_dic[db].split(','), line))
    ...
    try:
        ...
        with open("%s/%s" % (db_path, db[1]), 'r', encoding='utf-8') as f, \
            ...
            for line in f:
                ...
                if 'phone' in insert_key:
                    if insert_dict['insert_into']['phone'] in line[3]:
                        ...
                        write_f.close()
                        os.remove("%s/%s" % (db_path, db[1]) + '~')
                        return logger.error("唯一索引phone已存在")
                count = int(line[0]) + 1
            ...
            for x in insert_values:
                lines.append(x)
            write_f.write(write_str.join(lines) + '\n')
            logger.warning("写入成功 %s" % lines)
        os.remove("%s/%s" % (db_path, db[1]))
        os.rename("%s/%s" % (db_path, db[1]) + '~',
                  "%s/%s" % (db_path, db[1]))
    except:
        logger.error("语法有误请重新输入")
    return
...
def delete_module(sql_str):
    ...
    try:
        ...
        with open("%s/%s" % (db_path, db[1]), 'r', encoding='utf-8') as f, \
            ...
            for line in f:
                line = line.split(',')
                db_dict = dict(zip(db_dic[db[1]].split(','), line))
                if screen_print(delete_dict['where'], db_dict):
                    logger.warning("已删除%s" % line)
                    continue
                write_f.write(write_str.join(line))
        os.remove("%s/%s" % (db_path, db[1]))
        os.rename("%s/%s" % (db_path, db[1]) + '~', "%s/%s" % (db_path, db[1]))
...
def set_module(set_value, db):
    ...
    line = []
    for k in range(len(db_dic[db].split(','))):
        line.append(k)
    line_num = dict(zip(db_dic[db].split(','), line))
    set_dicts = {}
    for k in set_dict:
        if k in line_num:
