...
from ATM.lib.atm_function import repayment
                        ...
                        if checkout(shopping_all_amt):
                            for key in shopping_cart:
                                sql = "insert into %s values (%s,%s,%s,%s,%s)" % (
                                    sql_pay, current_status['user'], key, shopping_cart[key]['num'],
...
def checkout(money):
    ...
    sql = "select money,limit from %s where login = %s" % (sql_emp, loginname)
    ...
    while True:
        if money > float(user_money['limit']):
            ...
            if inp == "Y":
                ...
                sql = "update %s set money = %.2f where login = %s" % (sql_emp, user_money['money'], loginname)
                sql_distribute(sql)
                sql1 = "insert into %s values (%s,%s,%s,%s,%s)" % (sql_bill, loginname, '消费', money, 0,
                                                                   time.strftime("%Y-%m-%d", time.localtime()))
                sql_distribute(sql1)
                return True
            elif inp == "N":
